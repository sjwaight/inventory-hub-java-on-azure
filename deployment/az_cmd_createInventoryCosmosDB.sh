...
az cosmosdb collection create --resource-group "$INVENTORY_HUB_APP_NAME""-main-dev" --name "$INVENTORY_HUB_APP_NAME""-cosmosdb-main-dev1" --db-name "inventory" --collection-name "product-items" --partition-key-path "/productId" --throughput 400
az cosmosdb collection create --resource-group "$INVENTORY_HUB_APP_NAME""-main-dev" --name "$INVENTORY_HUB_APP_NAME""-cosmosdb-main-dev1" --db-name "inventory" --collection-name "product-inventory" --partition-key-path "/location" --throughput 400
az cosmosdb collection create --resource-group "$INVENTORY_HUB_APP_NAME""-main-dev" --name "$INVENTORY_HUB_APP_NAME""-cosmosdb-main-dev1" --db-name "inventory" --collection-name "transaction-events" --partition-key-path "/type" --throughput 400
